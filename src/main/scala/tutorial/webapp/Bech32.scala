...
case object Bech32 {
  ...
  final val CHARSET_MAP: Map[Char, Int5] = CHARSET.zipWithIndex.view.toMap.mapValues(_.toByte).toMap
...
//    require(l >= 8 && l <= 90, s"Invalid Bech32: $bech32 (length $l). Valid length range: 8-90 characters.")
    require(l >= 8 , s"Invalid Bech32: $bech32 (length $l). Valid length gt: 8 characters.")
